...
architecture rtl of baseline_restorer is
    ...
    -- Baseline is the FASTER stream -> delay it to meet the slower Jordanov stream
    constant C_LATENCY_SKEW : natural := G_JORD_LATENCY - G_MOV_LATENCY;

    -- Widths: baseline/ADC = G_DATA_WIDTH (unsigned); Jordanov/output = G_DATA_WIDTH+1 (signed)
    constant C_JORD_WIDTH : natural := G_DATA_WIDTH + 1;
    ...
    signal baseline_skew : t_skew_arr;
    ...
    signal diff_ext      : signed(C_JORD_WIDTH downto 0); -- 1 guard bit above the 15-bit result

    signal data_out    : std_logic_vector(C_JORD_WIDTH - 1 downto 0); -- 15-bit signed
...
begin
    ...
    begin
        if (RST_N_I = '0') then
            baseline_skew <= (others => (others => '0'));
        elsif rising_edge(CLK_I) then
            if (CE_I = '1') then
                baseline_skew(0) <= BASELINE_I;
                for i in 1 to C_LATENCY_SKEW loop
                    baseline_skew(i) <= baseline_skew(i - 1);
                end loop;
            end if;
        end if;
    end process p_skew;
        ...
        if (RST_N_I = '0') then
            ...
            if (CE_I = '1') then
                if (LATCH_TRIG_I = '1') then
                    baseline_held <= baseline_skew(C_LATENCY_SKEW);
                end if;
            end if;
        end if;
    end process p_latch;
    ...
    begin
        if (RST_N_I = '0') then
            ...
            if (CE_I = '1') then
                ...
                diff_ext <= resize(signed(DATA_JORD_I), C_JORD_WIDTH + 1)
                    - resize(signed('0' & baseline_held), C_JORD_WIDTH + 1);
                -- truncate back to 15-bit signed output
                data_out <= std_logic_vector(resize(diff_ext, C_JORD_WIDTH));
            end if;
        end if;
    ...
    begin
        if (RST_N_I = '0') then
            ...
            if (CE_I = '1') then
                if (diff_ext(C_JORD_WIDTH) /= diff_ext(C_JORD_WIDTH - 1)) then
                    error_oflow <= '1';
                end if;
            end if;
        end if;
