...
--  Designed for unsigned input and signed output with a latency of 8 cycles.
--
...
--    Md_full[n] = M_scaled * diff[n]                         (+2 cycles: DSP multiplication)
--    acc1_sc[n] = acc1[n] << M_FRAC                          (+3 cycles: Aligned branches)
--    acc2[n] = acc2[n-1] + acc1_sc[n] + Md_full[n]           (+4 cycles: Running sum 2)
--
--    Normalization:
--    acc2_shift[n] = acc2[n] << C_NORM_SHIFT                 (+5 cycles: Shift to DSP width)
--    norm_prod[n] = acc2_shift[n] * C_NORM_COEF              (+6 cycles: Divide by scale factor G)
--    norm_round[n] = round(norm_prod)                        (+7 cycles: Round output)
--    y[n]   = saturate (norm_round)                          (+8 cycles: Saturate output)
-- 
--  Dependencies:
...
architecture rtl of jordanov_filter is
    ...
    constant C_ACC1_SC_WIDTH       : natural := C_ACC1_WIDTH + C_M_EXP_FRAC_WIDTH;                               -- acc1_q0: increased acc1 width for M*diff lsb weight: acc1 (25b) + M_FRAC (4b) = 29b
    constant C_ACC2_WIDTH          : natural := C_MDIFF_WIDTH + G_ACC2_MARGIN_BITS + C_K_WIDTH;                  -- acc2: M*diff (35b) + acc1_sc (29b) + margin (1b) + integ k (8b) = 44b
    ...
    constant C_M_EXP_FRAC_ZEROS : std_logic_vector(C_M_EXP_FRAC_WIDTH - 1 downto 0) := (others => '0');                         -- Padding for acc1 into the M*diff width
    ...
    -- Gain of the filter: G = k * (2^M_FRAC + M_VALUE) in acc2 width
    constant C_M_EXP_FRAC_VALUE : natural := 2 ** C_M_EXP_FRAC_WIDTH;                          -- M_exp fractional scaling factor, kept inside the accumulator
    constant C_GAIN_NUM         : natural := G_K_DELAY * (C_M_EXP_FRAC_VALUE + G_M_EXP_VALUE); -- Gain as exact integer (natural)
    constant C_GAIN_REAL        : real    := real(C_GAIN_NUM);                                 -- Gain as a real in compilation
    ...
    signal Mdiff_q0 : std_logic_vector(C_MDIFF_WIDTH - 1 downto 0);   -- M*diff + 1 cycle
    signal acc1     : std_logic_vector(C_ACC1_WIDTH - 1 downto 0);    -- Accumulator 1
    signal acc1_q0  : std_logic_vector(C_ACC1_SC_WIDTH - 1 downto 0); -- Accumulator 1 increased to M*diff full width
...
begin
    ...
    begin
        if (RST_N_I = '0') then
            ...
            -- differentiator runs at + 1 cycle
    ...
    p_acc1 : process (CLK_I, RST_N_I)
        variable v_acc1 : signed(C_ACC1_WIDTH downto 0);
    begin
        if (RST_N_I = '0') then
            acc1 <= (others => '0');
        elsif rising_edge(CLK_I) then
            -- accumulator1 runs at + 2 cycles
            v_acc1 := resize(signed(acc1), v_acc1'length)
                + resize(signed(diff), v_acc1'length);
            -- saturate instead of wrap
            acc1 <= std_logic_vector(f_sat_resize(v_acc1, acc1'length));
        end if;
    end process p_acc1;

    -- STAGE 2: Pole zero multiply (Mdiff = M_exp * diff)
    p_mult : process (CLK_I, RST_N_I)
    begin
        if (RST_N_I = '0') then
            Mdiff <= (others => '0');
        elsif rising_edge(CLK_I) then
            -- multiplier runs at + 2 cycles
            Mdiff <= std_logic_vector(C_M_EXP_FULL_VALUE * signed(diff));
        end if;
    end process p_mult;

    -- STAGE 3: Delay of acc1 and Mdiff of 1 cycle
    ...
    begin
        if (RST_N_I = '0') then
            ...
            -- delays from stage 3 to stage 4
            acc1_q0  <= acc1 & C_M_EXP_FRAC_ZEROS;
            Mdiff_q0 <= Mdiff;
        end if;
    end process p_reg;

    -- STAGE 4: Accumulator 2 (acc2[n] = acc2[n-1] + acc1_sc[n] + Mdiff[n])
    p_acc2 : process (CLK_I, RST_N_I)
    begin
        if (RST_N_I = '0') then
            acc2 <= (others => '0');
        elsif rising_edge(CLK_I) then
            -- acc2 runs at + 4 cycles
            acc2 <= std_logic_vector(signed(acc2)
                + resize(signed(acc1_q0), acc2'length)
                + resize(signed(Mdiff_q0), acc2'length));
        end if;
    end process p_acc2;
